...
const displayClan = async (ctx: IBotContext) => {
    ...
    if (!clanDetails || !clanDetails.isInClan || typeof clanDetails.clanDetails === 'undefined') {
        ctx.reply("Вы пока не состоите в клане!\n", Markup.inlineKeyboard([
    ...
    const clanRank = topClans.findIndex(clan => clan.id === clanDetails.clanDetails?.id) + 1;
...
clanScene.action('display_clan', async ctx => {
    ...
    const clanDetails = await UserService.checkForClan(ctx.from.id);
    if (!clanDetails || !clanDetails.isInClan || typeof clanDetails.clanDetails === 'undefined') {
        ctx.editMessageText("Вы пока не состоите в клане!\n", Markup.inlineKeyboard([
            [Markup.button.callback('Создать клан', 'create_clan'), Markup.button.callback('Лучшие кланы', 'leaderboard')],
            [Markup.button.callback('Назад', 'back_to_menu')]
        ]));
        return;
...
clanScene.action('leave_clan', async ctx => {
    ...
    const buttons = [
        Markup.button.callback('Вернуться', 'display_clan')
    ];
...
clanScene.action("leaderboard", async (ctx) => {
    try {
        ...
        ctx.editMessageText(messageText, Markup.inlineKeyboard([Markup.button.callback('Назад', "display_clan")]));
